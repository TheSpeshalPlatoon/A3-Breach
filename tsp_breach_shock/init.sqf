tsp_fnc_breach_shock_wire = {
	...
	if (!isNil "tsp_fnc_animate_clacker") then {[_unit, "", "tsp_animate_shock_loop_wnon", "\tsp_breach_shock\shock.p3d", [0.02,0,0], [0,0,-100], {!("shock" in gestureState playa)}, false] spawn tsp_fnc_gesture_item};
	_helper_ammo = "ace_fastroping_helper" createVehicle [0,0,0]; _helper_ammo enableSimulation false; _helper_ammo allowDamage false; 
	...
	{[_helper_unit, _x] remoteExec ["disableCollisionWith", _x]} forEach allPlayers;
	...
	_helper_ammo enableSimulation true; _helper_ammo setVelocityModelSpace [0,-15,0]; detach _helper_unit;
	sleep tsp_cba_breach_shock; {deleteVehicle _x} forEach [_helper_ammo, _helper_unit]; 
};

tsp_fnc_breach_shock_wave = {
	params ["_unit", "_explosive", "_magazine", "_fuzeTime", "_triggerItem", ["_helpers", attachedObjects (_this#0) select {typeOf _x == "ace_fastroping_helper"}]];
