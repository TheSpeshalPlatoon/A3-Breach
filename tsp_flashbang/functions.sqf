tsp_fnc_flashbang = {
	params ["_thrower", "_flash", ["_smoke", true], ["_distance", tsp_cba_flashbang_distance], ["_volume", tsp_cba_flashbang_volume], ["_sound", "tsp_flashbang\snd\bang.ogg"]];
	playSound3D [_sound, _flash, false, getPosASL _flash, _volume, (random 1) min 1.3 max 0.7, 250];  //-- Sound
	_flash addTorque [(random 20)-10, (random 20)-10, (random 20)-10];        //-- Fly
	_flash setVelocityModelSpace [(random 8)-4, (random 8)-4, random 4];     //-- Fly
	[_flash, _smoke] remoteExec ["tsp_fnc_flashbang_effects", 0];  //-- Global effects
	{  //-- Victims
	...
	} forEach (allUnits select {(_x distance _flash < _distance/5) || (_x distance _flash < _distance && [_flash, "VIEW"] checkVisibility [eyePos _x, eyePos _flash] > 0)});
};

tsp_fnc_flashbang_effects = {  //-- Particle effects, global
	params ["_flash", ["_smokeFX", true], ["_intensity", 1]];
	...
	_sparks = "#particleSource" createVehicleLocal [0,0,0];	if (tsp_cba_flashbang_sparks) then {_sparks attachto [_flash, [0,0,0]]}; _sparks setParticleClass "FireSparksSmall";    //-- Sparks    
	_fire = "#particleSource" createVehicleLocal [0,0,0]; if (tsp_cba_flashbang_fire) then {_fire attachto [_flash, [0,0,0]]}; _fire setParticleClass "ScudFire";                  //-- Fire    
	_light = "#lightpoint" createVehicleLocal (getPos _flash);  //-- Light
	_light setLightColor [1,0.8,0.5]; _light setLightAmbient [1,0.8,0.5];
	_light setLightUseFlare true; _light setLightFlareSize (_intensity*tsp_cba_flashbang_brightness); 
	_light setLightFlareMaxDistance (_intensity*tsp_cba_flashbang_brightness);
	_light setLightBrightness (_intensity*tsp_cba_flashbang_brightness); _light setLightDayLight true;
